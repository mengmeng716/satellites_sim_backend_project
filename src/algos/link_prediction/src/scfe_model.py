...
class SCFE_without_Future_Clean(nn.Module):
    def __init__(self, input_len=30, input_dim=11, output_len=30, hidden_dim=512, output_dim=1):
        """
        input_dim=11 : 链路版（与训练集 X_train.npy 严格对齐）
        output_dim=1 : 链路版，单条链路流量预测
        """
        super().__init__()
    ...
    def forward(self, history_data, future_info=None):
        # history_data: (batch, input_len, input_dim)
        ...
        # 重塑为 (batch, output_len, output_dim)
